...
- (void)example12
{
    ...
    [self.tableView addGifFooterWithRefreshingTarget:self refreshingAction:@selector(loadMoreData) offsetBottom:0];
...
- (void)example15
{
    ...
    [self.tableView addLegendFooterWithRefreshingTarget:self refreshingAction:@selector(loadLastData) offsetBottom:0];
...
- (void)example16
{
    // 添加传统的上拉刷新
    // 设置回调（一旦进入刷新状态，就调用target的action，也就是调用self的loadMoreData方法）
    [self.tableView addLegendFooterWithRefreshingTarget:self refreshingAction:@selector(loadMoreData) offsetBottom:0];
...
- (void)example18
{
    ...
    [self.tableView addLegendFooterWithRefreshingTarget:self refreshingAction:@selector(loadOnceData) offsetBottom:0];
}
