...
- (UIView *)markImageView
{
    if (!_markImageView) {
        UIView *markImageView = [[UIView alloc] init];
...
- (void)layoutSubviews
{
    ...
    if (self.updatedTimeHidden) { // 显示状态
        ...
        // 1.状态标签
        _stateLabel.frame = CGRectMake(0, 2, stateW, stateH - 2);
        
        // 2.时间标签
        CGFloat updatedTimeY = stateH - 3;
...
- (void)setUpdatedTime:(NSDate *)updatedTime
{
    ...
    if (updatedTime) {
        ...
        self.updatedTimeLabel.text = @"最后更新：从未刷新";
    }
}
